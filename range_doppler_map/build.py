#!/usr/bin/env python3
"""
Build the Short "What Is a Range-Doppler Map?" end to end.

    python build.py                # full 1080x1920 60fps build
    python build.py --quality m    # faster 720x1280 30fps draft
    python build.py --skip-voice   # reuse existing voiceovers/
    python build.py --only Short3  # rebuild a single scene

Pipeline
    1. voice.py  -> voiceovers/ShortN.mp3 + durations.json
    ...
    3. ffmpeg    -> mux each scene with its MP3 (audio silence-padded to the
                    exact video length), then concatenate all six
...
PARTS = [f"Short{i}" for i in range(1, 7)]
STEM = "short_what_is_a_range_doppler_map"
FINAL = os.path.join(OUT_DIR, STEM + ".mp4")
SRT = os.path.join(OUT_DIR, STEM + ".srt")

# Vertical Shorts render at 9:16; Manim names the media folder by pixel height
# plus fps, so the resolution flag and the folder name have to agree.
QUALITY = {"l": ("480,854", "854p15"),
           "m": ("720,1280", "1280p30"),
           "h": ("1080,1920", "1920p60")}
...
def scene_path(part, quality):
    return os.path.join(MEDIA_DIR, "videos", "short", QUALITY[quality][1], f"{part}.mp4")
...
def step_render(quality, only):
    print(f"\n[2/4] sahneler render ediliyor (-q{quality}, {QUALITY[quality][0]})")
    parts = [only] if only else PARTS
    run([PY, "-m", "manim", f"-q{quality}", "--disable_caching",
         "-r", QUALITY[quality][0], "--media_dir", MEDIA_DIR,
         os.path.join(HERE, "short.py"), *parts], cwd=HERE)
...
def step_mux(quality):
    ...
    for part in PARTS:
        ...
        # apad tops the narration up with silence to the exact picture length, so
        # audio never runs past the cut and never gets truncated either.
    ...
    print(f"  toplam sure    : {total:.2f}s")
    if total > 60:
        print("  !! 60 saniyeyi asti — Shorts icin anlatimi kisalt")
...
def step_subtitles(report):
    ...
    def cues(text, limit=62):
        """Sentences, split further at commas so no cue overruns a Short's width."""
        out = []
    ...
    for part, vdur, adur, _ in report:
        sentences = cues(" ".join(SCRIPTS[part].split()))
...
def main():
    ...
    ap.add_argument("--only", help="render a single scene, e.g. Short3")
    args = ap.parse_args()
    ...
    print("\n=== TAMAMLANDI ===")
    print(f"  {final}")
    print(f"  sure {dur:.1f}s  ·  {size:.1f} MB  ·  {QUALITY[args.quality][0].replace(',', 'x')}")
