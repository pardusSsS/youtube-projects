#!/usr/bin/env python3
"""
Build "What Does a Radar Actually See? | From Chirp to Detection" end to end.
    ...
    python build.py --only Part7   # rebuild a single scene
...
Pipeline
    ...
    3. ffmpeg    -> mux each scene with its MP3 (audio silence-padded to the
                    exact video length), then concatenate all sixteen
    4. SRT       -> sentence-level subtitles on the real timeline

The render log is scanned for the [LAYOUT] lines that main.py emits whenever two
pieces of text overlap or something drifts off the frame; anything found is
reprinted at the end of the build instead of disappearing up the scrollback.
"""
...
PARTS = [f"Part{i}" for i in range(1, 17)]
QUALITY_DIR = {"l": "480p15", "m": "720p30", "h": "1080p60", "k": "2160p60"}
STEM = "what_does_a_radar_actually_see"
...
def step_voice(skip):
    ...
    run([PY_EXE, os.path.join(HERE, "voice.py")], cwd=HERE)
...
def step_render(quality, only):
    ...
    cmd = [PY_EXE, "-m", "manim", f"-q{quality}", "--disable_caching",
    ...
    for line in proc.stdout:
        ...
        if line.startswith("[LAYOUT]"):
            ...
            sync.append(line.rstrip())
    if proc.wait() != 0:
        raise SystemExit("manim render basarisiz")
    return layout, sync
...
def step_mux(quality):
    ...
    for part in PARTS:
        vid = os.path.join(MEDIA_DIR, "videos", "main", QUALITY_DIR[quality],
                           f"{part}.mp4")
...
# ------------------------------------------------------------------ description
def step_description(report):
    """YouTube description with chapters on the real timeline."""
    titles = [
        "The question", "The chirp", "The delayed echo", "The mixer",
        "What lands in memory", "The range FFT", "A peak is not a size",
        "One frame of chirps", "The Doppler FFT", "The range-Doppler map",
        "The angle FFT", "A cube of energy", "Noise and clutter",
        "CFAR", "Detections to tracks", "The whole chain",
    ]
    path = os.path.join(OUT_DIR, "youtube_description.txt")
    clock, chapters = 0.0, []
    for (part, vdur, _, _), title in zip(report, titles):
        m, s = divmod(int(clock), 60)
        chapters.append(f"{m:d}:{s:02d}  {title}")
        clock += vdur
    with open(path, "w") as f:
        f.write("What Does a Radar Actually See? | From Chirp to Detection\n\n")
        f.write("A radar does not see a car. It sees a voltage. This is the whole\n"
                "chain that turns that voltage into a detection: the transmitted\n"
                "chirp, the delayed echo, the mixer, three Fourier transforms\n"
                "(range, Doppler, angle), the CFAR threshold, and finally the\n"
                "clusters and tracks you would actually recognise.\n\n")
        f.write("Chapters\n" + "\n".join(chapters) + "\n")
    print(f"  aciklama: {path}")
...
def main():
    ...
    if not args.skip_render:
        layout, _ = step_render(args.quality, args.only)
    if args.only:
    ...
    if layout:
        print(f"\n  !!! {len(layout)} yerlesim uyarisi:")
        for l in layout:
            print("   ", l)
    else:
        print("  yerlesim denetimi: temiz (cakisan yazi yok)")
